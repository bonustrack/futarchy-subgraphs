...
class GqlEntityController {
    ...
    generateQueryFields(schemaObjects, resolvers = {
        ...
        schemaObjects.forEach(type => {
            queryFields[(0, graphql_2.singleEntityQueryName)(type)] =
                this.getSingleEntityQueryConfig(type, resolvers.singleEntityResolver);
            queryFields[(0, graphql_2.multiEntityQueryName)(type)] =
                this.getMultipleEntityQueryConfig(type, resolvers.multipleEntityResolver, generatedFilters);
        });
        return queryFields;
    }
    /**
        ...
        return this.schemaObjects.reduce((entities, obj) => {
            entities[obj.name] = this.getTypeFields(obj).reduce((resolvers, field) => {
                ...
                if ((0, graphql_1.isListType)(nonNullType)) {
                    const itemType = (0, graphql_2.getNonNullType)(nonNullType.ofType);
                    if (itemType instanceof graphql_1.GraphQLObjectType) {
                        resolvers[field.name] = (0, resolvers_1.getNestedResolver)((0, graphql_2.multiEntityQueryName)(itemType));
                    }
                }
                if (nonNullType instanceof graphql_1.GraphQLObjectType) {
                    resolvers[field.name] =
                        fields[(0, graphql_2.singleEntityQueryName)(nonNullType)].resolve;
                }
                return resolvers;
            }, {});
            return entities;
        ...
        this.schemaObjects.map(type => {
            ...
            builder = builder
                ...
                this.getTypeFields(type).forEach(field => {
                    const fieldType = field.type instanceof graphql_1.GraphQLNonNull
                        ? field.type.ofType
                        : field.type;
                    if ((0, graphql_1.isListType)(fieldType) &&
                        fieldType.ofType instanceof graphql_1.GraphQLObjectType &&
                        (0, graphql_2.getDerivedFromDirective)(field)) {
                        return;
                    }
            ...
            if (knex.client.config.client === 'pg') {
                builder = builder.raw(knex
                    .raw('ALTER TABLE ?? ADD EXCLUDE USING GIST (id WITH =, _indexer WITH =, block_range WITH &&)', [tableName])
                    .toQuery());
            }
        });
        await builder;
...
# your defined entity.
    ...
    getSingleEntityQueryConfig(type, resolver) {
        return {
            ...
            args: {
                id: {
                    type: new graphql_1.GraphQLNonNull(this.getEntityIdType(type))
                },
                indexer: {
    ...
    getMultipleEntityQueryConfig(type, resolver, generatedFilters = {}) {
        const getWhereType = (nestedType, prefix) => {
            const name = prefix
                ? `${prefix}_${nestedType.name}_filter`
                : `${nestedType.name}_filter`;
            
            if (generatedFilters[name]) {
                return {
                    where: { type: generatedFilters[name] },
                    orderByValues: {}
                };
            }
            ...
            this.getTypeFields(nestedType).forEach(field => {
                ...
                if (nonNullFieldType instanceof graphql_1.GraphQLObjectType) {
                    ...
                    if (idField &&
                        ...
                        whereInputConfig.fields[`${field.name}_`] = getWhereType(nonNullFieldType, nestedType.name).where;
                        nonNullFieldType = (0, graphql_2.getNonNullType)(idField.type);
                    }
                }
                if ((0, graphql_1.isListType)(nonNullFieldType)) {
                    const itemType = nonNullFieldType.ofType;
                    if (!(0, graphql_1.isLeafType)(itemType)) {
                        return;
                    }
                    ...
                    if (itemType === graphql_1.GraphQLString) {
                ...
                if ((nonNullFieldType instanceof graphql_1.GraphQLScalarType &&
            ...
            const inputObjectType = new graphql_1.GraphQLInputObjectType(whereInputConfig);
            generatedFilters[name] = inputObjectType;
            
            const result = {
                where: { type: inputObjectType },
                orderByValues
            };
        ...
        return {
            type: new graphql_1.GraphQLNonNull(new graphql_1.GraphQLList(new graphql_1.GraphQLNonNull(type))),
            args: {
                first: {
                    type: graphql_1.GraphQLInt
                },
    ...
    generateSchema(opts) {
        ...
        const coreQueryFields = this.generateQueryFields([
            _1.MetadataGraphQLObject,
            _1.CheckpointsGraphQLObject
        ]);
